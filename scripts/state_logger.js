...
// ---------- encoding fix ----------
// TradingView's Pine table cells expose text with cp1252 byte semantics:
// each character's code point equals the corresponding UTF-8 byte value,
// but code points 0x80-0x9F are mapped through Windows-1252 (not ISO-8859-1).
// Re-interpret each character as a cp1252 byte and decode as UTF-8.
const _CP1252_REVERSE = new Map([
...
function fixCdpText(str) {
  if (!str || typeof str !== 'string') return str;
  if (!/[^\x00-\x7F]/.test(str)) return str; // pure ASCII — nothing to fix
  const bytes = [];
  for (let i = 0; i < str.length; i++) {
    ...
    if (cc < 0x100) {
      ...
      const b = _CP1252_REVERSE.get(cc);
      if (b !== undefined) bytes.push(b);
      // else: high-code-point char outside cp1252 table — skip
    }
  }
  const decoded = Buffer.from(bytes).toString('utf8');
  // If decoding produced replacement characters (U+FFFD), the input was already
  // correct Unicode and did not need fixing — return the original string.
...
async function getPineTables(s, studyFilter) {
  ...
  return raw.map((sObj) => {
    ...
    for (const item of sObj.items) {
      ...
      tables[tid][v.row][v.col] = v.t ? fixCdpText(v.t) : '';
    }
